...
<div class="frame_900_rec">
	<form id="changerMotPassForm" action="/clients/changerPass" method="Post">
	<input type="hidden" name="data[client_id]"   value="<?php echo $client_id ; ?>" />
	<input type="hidden" name="data[recover_code]"  value="<?php echo $recover_code ; ?>" />
   <table>
      <tr>
	    <td>Nouveau mot de passe :  </td>
		<td> <input type="password" id="password" name="data[Client][password]" class="validate[required]"  />		</td>
	  </tr>
	  <tr>
	    <td>Confirmer le nouveau mot de passe :  </td>
		<td> <input type="password" id="repassword" name="data[Client][repassword]" class="validate[required,equals[password]]"  />		</td>
	  </tr>
	  <tr>
