...
class SearchesController extends SearchAppController
{
	...
	function beforeFilter()
	{
		...
		{
			if(in_array($mode, array('natural', 'boolean','patternMatching')))
			{
				$this->mode = $mode;
			}
		}
	...
	function index($q = null)
	{
		...
		switch($this->mode)
		{
			...
			case 'patternMatching':
			   // $statement = $this->_prepareForPatternMatchingMode($q);
			    $statement = $q;
			    $conditions = 'Search.data LIKE \'%'.$statement.'%\'';
			    $order = 'modified DESC, created DESC';
			    break;
		}
		...
		$this->set('balise_h1', "Résultat de la recherche : " . $q );
		$this->set('page_title', "Résultat de la recherche : " . $q. " ~ Services4all");
		$this->set('is_page_search',true) ; 
		$this->set('data', $data);
	}
