    ...
    var $paginate = array(
							...
							'recursive'=>1,
									'fields'=>"Fournisseur.id, 
											   ...
											   Fournisseur.tel,    
											   Fournisseur.siege_social,  
											   Fournisseur.patente,  
											   Fournisseur.cnss,  
											   Fournisseur.email, 
											   Fournisseur.deleted" ,
							'order' => "Fournisseur.id desc" 
		);
...
//----------------------------------------- Admin ----------------------------------------------	
		...
		$this->paginate['conditions']["Fournisseur.deleted"] = 0   ; 
		$this->paginate['limit'] = 10 ; 
		$this->paginate['order'] = 'Fournisseur.id desc';
		$this->set('fournisseurs',$this->paginate('Fournisseur') );
		$this->set('balise_h5',"Liste des Fournisseurs");
	...
	function admin_ajouter(){				
		if(isset($this->data)) {
	       	if( $this->Fournisseur ->save($this->data) ){
				$this->data['Fournisseur']['user_id'] = $this->Session->read('Auth.User.id') ;	
				...
				$this->Fournisseur->save($this->data);		
				$this->Session->setFlash("
					<button class='close' aria-label='Close' data-dismiss='alert' type='button'>
						<span aria-hidden='true'>×</span>
					</button>
					Le nouveau fournisseur a bien été ajouté.",
					"default",array('class'=>'alert alert-default bg-success-300','role'=>'alert'));
				$this->redirect('/admin/fournisseurs/index');
			}else{
				$this->Session->setFlash("
				<button class='close' aria-label='Close' data-dismiss='alert' type='button'>
					<span aria-hidden='true'>×</span>
	...
	function admin_modifier ($id){ 
	        ...
	        //if(isset($this->data) ) {
			//if ($this->Session->read('Auth.User.id')!= $this->data['Fournisseur']['user_id']){//on vérifie si l'utilisateur a le droit de modifier cette demande (donc une demande qui lui appartient)
			//$this->Session->setFlash("
			//		<button class='close' aria-label='Close' data-dismiss='alert' type='button'>
			//			<span aria-hidden='true'>×</span>
			//		</button>
		...
		if($this->Fournisseur->save($this->data))  {
			...
			$this->Session->setFlash("
				...
				"default",array('class'=>'alert alert-default bg-success-300','role'=>'alert'));
			$this->redirect('admin/fournisseurs/index'.$id) ;
			}
		 
		$this->data = $this->Fournisseur->read(array(),$id) ;

		$q = $this->Fournisseur->findById($id)  ;
		$this->set('fournisseur', $q );
	...
	//	$this->loadModel('TypeMateriel');		
		$Fournisseur_id = $this->Fournisseur->find('first',array('conditions'=>array('Fournisseur.id'=>$id)));	

		$this->set('fournisseur',$q) ;

		$this->set('fournisseur_id',$id) ; 

		$page_title = "Détail de Fournisseur N° ".$q["Fournisseur"]["id"] ;
	...
	function admin_rechercher() {
		...
		//$this->paginate['conditions']["Materiel.statut"] =  1 ;  
		if( !empty($this->data) ) {
	...
	function admin_supprimer($id) {
		if($id!=null){
            ...
            if($this->Fournisseur->saveField('deleted',1) ) {
			   $this->Session->setFlash("
						<button class='close' aria-label='Close' data-dismiss='alert' type='button'>
							<span aria-hidden='true'>×</span>
						</button>
