...
if (!defined('IN_CRAWLT'))
	...
	require_once("/home/services/public_html/app/webroot/crawlprotect/include/connection.php");
		...
		foreach($_POST as $key=> $value)
			...
			foreach($tabforbiddenword as $word)
				...
				if(preg_match("/".$word."/i", $value) && $word!='')
					{
					header("Location: http://www.services4all.ma/crawlprotect/noaccess/index.php?crawlprotecttype=spammer&crawlprotectcontent=".$word."&crawlprotecturl=".$_SERVER['REQUEST_URI']."");
					exit;			
					}	
				}
			...
			foreach($listsql as $sql)
				...
				if( (preg_match("/".$sql."/i", $value) && (preg_match("/(\%27)|(\')/i", $value)|| preg_match("/((\%3D)|(=))[^\n]*((\%27)|(\')|(\-\-\s)|(\%3B)|(;))/i", $value))) || preg_match("/\w*((\%27)|(\'))((\%6F)|o|(\%4F))((\%72)|r|(\%52))/i", $value)|| preg_match("/(\-\-\s)|(\'\/\*)/i", $value) )
					{
					header("Location: http://www.services4all.ma/crawlprotect/noaccess/index.php?crawlprotecttype=sqlinjection&crawlprotecturl=".$_SERVER['REQUEST_URI']."&crawlprotect2contentvariable_POST=".urlencode($value)."");
					exit;			
					}	
				}			
